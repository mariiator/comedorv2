...
function obtenerAlumnos($clase){
	...
	try {
		...
		foreach($informacion as $row){
			...
			$foto = $row[4];
			$array[$id_persona] = array($foto, join(" ",$nom_persona),$row[5]);
		}
		if(!empty($array)){
			return $array;
		}
...
function mostrarTablas($clase){
	...
	$tabla2 = "";
	foreach($alumnos as $id_persona => $valores){
		$size = "";
		//EN CASO DE QUE EL NOMBRE SEA MUY LARGO PARA QUE ENTRE DENTRO DEL BOTON
		if(strlen($valores[1]) > 34){ 
			$size = "style='font-size:12px;'";
		}
		if($valores[2] == "0"){
			$tabla .= '
			<div id="'.$id_persona.'" class="container-2 ausente" onclick="javascript:doclick(\'/comedorv2/user/controlador/registrar.php\',\'#'.$id_persona.'\');">
				<a class="thumbnail" >
				<div class="image-2">
					<div class="image-overlap">
	    				<img class="alumno" id="'.$id_persona.'" src="data:image/png;base64,'.$valores[0].'" height="100%" onerror="imgError(this);">
					</div>
					<div class="bottom-overlap-2 transparent-back">
						<small '.$size.'>'.$valores[1].'</small>
					</div>
				</div>
				</a>
			</div>';
			...
			$tabla2 .= '
			<div id="'.$id_persona.'" class="container-2 asistente" onclick="javascript:doclick(\'/comedorv2/user/controlador/anular.php\',\'#'.$id_persona.'\');">
				<a class="thumbnail" >
				<div class="image-2">
	...
	$tabla .= '
			<div class="container-2" onclick="javascript:clickAll();" id="marcarTodo">
				<a class="thumbnail">
					<div class="image-2">
						<div class="image-overlap">
							<img src="/comedorv2/css/clase.png" height="100%">
						</div>
						<div class="bottom-overlap-2 transparent-back">
							<b>MARCAR TODOS</b>
						</div>
