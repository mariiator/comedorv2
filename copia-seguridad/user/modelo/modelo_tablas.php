...
function obtenerPersonas($etapa) {
	...
	try {
		$obtenerInfo = $conexion->prepare("
			SELECT Clase.nombre, Persona.Nombre, Persona.apellido1, Persona.apellido2
				FROM Etapa
...
function obtenerAsistencia($etapa){
	...
	try {
		$obtenerInfo = $conexion->prepare("
			SELECT Clase.nombre, Persona.Nombre, Persona.apellido1, Persona.apellido2, Registro.hora
				FROM Etapa
					INNER JOIN Clase ON Etapa.id = Clase.etapa_id
					INNER JOIN Persona ON Persona.clase_id = Clase.id
...
function obtenerFaltas($total,$asistencia){
	$array = array();
	foreach($total as $alumno => $clase){
		if (!array_key_exists($alumno, $asistencia)) {
	        $array[$alumno] = $clase;
	    }
	}
	return $array;
...
function mostrarTabla($array){
	...
	foreach($array as $alumno => $clase){
		$tabla .= "<tr><td>".$clase."</td><td>".$alumno."</td></tr>";
	}
	$tabla .= "</table>";
	return $tabla;
}
...
function mostrarTablaAsistencia($array){
	...
	foreach($array as $alumno => $valor){
		$tabla .= "<tr><td>".$valor[0]."</td><td>".$alumno."</td><td>".$valor[1]."</tr>";
	}
	$tabla .= "</table>";
	return $tabla;
}
