...
function obtenerCalendario($id,$dia){
	global $conexion;
	$array = array();
	try {
		$obtenerInfo = $conexion->prepare("SELECT Id_Profesor,Dia,Tipo_menu FROM Asistencias_profe WHERE Id_Profesor = :id AND Dia = :dia;");
		$obtenerInfo->bindParam(':id', $id);
		$obtenerInfo->bindParam(':dia', $dia);
		$obtenerInfo->execute();
...
function obtenerDiasEntreSemana($mes, $anio) {
	...
	$diaDeLaSemanaPrimerDia = date('N', $primerDia);

	// Agregar los días del mes anterior si el primer día del mes no es sábado o domingo
	if ($diaDeLaSemanaPrimerDia < 6) {
		for ($i = $diaDeLaSemanaPrimerDia - 1; $i > 0; $i--) {
			$diaAnterior = strtotime("-$i days", $primerDia);
			$diasEntreSemana[] = date('Y-m-d', $diaAnterior);
		}
	}

	// Recorrer todos los días del mes y agregar solo los días entre semana
...
function obtenerPdf($mes,$anyo,$tipo){
	global $conexion;
    try {
        $obtenerInfo = $conexion->prepare("SELECT Menu FROM Menus WHERE Mes = :mes AND Anyo = :anyo AND tipo_menu = :tipo;");
...
function mostrarCalendario($diasEntreSemana, $mes, $fecha) {
	$id = $_COOKIE['profesor'];
    ...
    foreach ($diasEntreSemana as $dia) {
    	...
    	$day = $explodeFecha[2];
        $encontrado = false;
        $calendario = obtenerCalendario($id,$dia);
...
function consultarAlergias($correo){
	global $conexion;
	try {
		$obtenerInfo = $conexion->prepare("SELECT alergias FROM Alergias WHERE id_profesor = :id;");
		$obtenerInfo->bindParam(':id', $correo);
		$obtenerInfo->execute();
		$alergias=$obtenerInfo->fetchColumn();
		return $alergias;
	} catch (PDOException $ex) {
		echo $ex->getMessage();
		return false;
...
function guardarAlergia($alergia,$correo){
	global $conexion;
	try {
		$obtenerInfo = $conexion->prepare("SELECT * FROM Alergias WHERE id_profesor = :id;");
		$obtenerInfo->bindParam(':id', $correo);
		$obtenerInfo->execute();
		$informacion=$obtenerInfo->fetchAll();
		if(!empty($informacion)){
			$update = $conexion->prepare("UPDATE Alergias SET alergias = :alergias WHERE id_profesor = :id;");
			$update->bindParam(':id', $correo);
			$update->bindParam(':alergias', $alergia);
			$update->execute();
		}else{
			$insert = $conexion->prepare("INSERT INTO Alergias (alergias,id_profesor) VALUES (:alergias,:id);");
			$insert->bindParam(':id', $correo);
			$insert->bindParam(':alergias', $alergia);
			$insert->execute();
		}
		return true;
	} catch (PDOException $ex) {
